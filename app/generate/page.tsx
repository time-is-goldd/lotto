...
// dream 쿼리파라미터는 app/dream/[keyword]/page.tsx의 CTA가 붙여주는 선택적 표시용 정보일
// 뿐이다(docs/PHASE7_DREAM_NUMBER_INTEGRATION_REPORT.md §8) — 잘못됐거나 존재하지 않는 값이면
// 조용히 무시하고 일반 생성 흐름으로 폴백한다(§9 "일반 /generate 사용자 경험을 깨뜨리지
// 않는다"). 최종 저장 시 relatedDreamId 검증은 POST /api/numbers가 서버에서 다시 하므로
// (lib/api/numbers.ts의 saveUserNumbers), 여기서 실패해도 데이터 무결성 문제가 아니라 표시
// 문제일 뿐이다.
  ...
  return (
    <Container className="flex flex-col gap-8 py-10">
      ...
      <NumberGenerator
        ...
        dreamContext={dream ? { id: dream.id, keyword: dream.keyword } : null}
      />
    </Container>
  );
}
